  ...
  describe "GET :show" do
    ...
    shared_examples "does not fetch invoice" do
      ...
      it "does not fetch an invoice" do
        get url_under_test, headers: headers
        expect(GetUserInvoiceJob).not_to have_been_enqueued
        expect(GetUserInvoiceJob).not_to have_been_performed
      end
    ...
    context "when logged in as non-admin" do
      ...
      context "when prerequisites are met" do
        before(:each) { allow(GetUserInvoiceJob).to receive(:perform_now).and_return(result) }
        let(:authenticated_user) { create(:user, :with_openstack_details) }
        let!(:cloud_service_config) { create(:cloud_service_config) }
        let(:result) { GetUserInvoiceJob::Result.new(true, invoice_document, nil, 200) }
        ...
        it "fetches an invoice" do
          ...
          get url_under_test, headers: headers
          expect(GetUserInvoiceJob).to have_been_performed
        end
