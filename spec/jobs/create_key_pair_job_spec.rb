...
RSpec.describe CreateKeyPairJob, type: :job do
  ...
  subject { CreateKeyPairJob::Runner.new(key_pair: key_pair, config: config, user: user) }
  ...
  describe "#perform" do
    ...
    context "when request times out" do
      ...
      it "returns an unsuccessful result" do
        result = described_class.perform_now(key_pair, config, user, test_stubs: stubs, timeout: timeout)
        expect(result).not_to be_success
      end
