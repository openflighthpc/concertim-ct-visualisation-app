...
//    includeScrollOffsets: false,
    ...
    cumulativeOffset: function (element, include_scroll_offsets) {
        if (typeof(include_scroll_offsets) == "undefined" || include_scroll_offsets === null) {
          include_scroll_offsets = false
        }
        var orig_elem = element;
        var valueT = 0;
        var valueL = 0;
        do {
        ...
        if (this.includeScrollOffsets && include_scroll_offsets) {
          var offsetcache = this.realOffset(orig_elem);
          valueT -= offsetcache.y;
          valueL -= offsetcache.x;
          if (this.windowOffset) {
            valueT -= this.windowOffset.y;
            valueL -= this.windowOffset.x;
          }
        }
