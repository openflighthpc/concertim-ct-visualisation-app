...
# Delete the user.
#
# If the `recurse=true` get parameter is not provided, the user will only be
# deleted if it currently has no racks or devices. If the `recurse=true` get
# parameter is provided, all of the user's racks and devices will be deleted
# along with the user.
curl -s -k \
  -H 'Accept: application/json' \
  -H "Authorization: Bearer ${AUTH_TOKEN}" \
  -X DELETE "${BASE_URL}/users/${USER_ID}${PARAMS}"
